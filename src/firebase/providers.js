...
export const resgisterUserWithEmailPassword = async({email, password, displayName}) => {
    try {
        const resp = await createUserWithEmailAndPassword( FirebaseAuth, email, password);
        const { uid, phtotoURL } = resp.user;
        await updateProfile(FirebaseAuth.currentUser, {displayName});
        return {
            ok: true,
            uid, phtotoURL, email, displayName
        }
...
export const loginWithEmailPassword = async({email, password}) => {
    try {
        ...
        const { uid, phtotoURL, displayName } = resp.user;
        return {
            ok: true,
            uid, phtotoURL, displayName,
        }
